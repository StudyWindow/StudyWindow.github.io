import { read_user_profile_if_email,read_from_supabase_db, save_to_supabase_db,save_pomo_settings,read_tomodoro_settings } from "/static/js/utils/utility.js";
import config_global from "/static/js/utils/globals.js";

export default async function create_tomodoro(supabase){
const user_props =await read_user_profile_if_email(supabase)
const userId= user_props["user_id"]
let timerWorker = new Worker("/static/js/src/worker.js");
const IS_AUTH_USER=config_global["IS_AUTH_USER"]
if(IS_AUTH_USER){
  const data=await read_tomodoro_settings(supabase,userId)
  const res=data[0]['pomo_settings']
  localStorage.setItem('pomo-selected-task',res['pomo-selected-task'])
  localStorage.setItem('pomo-theme',res['pomo-theme'])
  localStorage.setItem('pomo-theme-accent',res['pomo-theme-accent'])
  localStorage.setItem('pomo-config',JSON.stringify(res['pomo-config']))
  localStorage.setItem('pomo-tasks',JSON.stringify(res['pomo-tasks']))
}
...
async function getRecords(time) {
	...
	return new Promise((resolve) => {
      ...
      if(IS_AUTH_USER){
        result= read_from_supabase_db(supabase,userId)
        resolve(result);
      }else{
...
function saveRecord(r) {
	let tr = db.transaction("records", "readwrite").objectStore("records")
  const addRec =tr.add(r);
  config_global['STREAK']+= r['t']
	addRec.onsuccess = () => console.log("Added new record!");
  const getRec = tr.getAll()
  getRec.onsuccess=function(ev){
    const sw_data = {sw_db:ev.target.result}
    save_to_supabase_db(supabase,userId,sw_data,config_global['STREAK'])
  }
}
...
function saveTasks() {
  ...
  save_pomo_settings(supabase,userId);
}
